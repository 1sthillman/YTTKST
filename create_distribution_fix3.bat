...
:: Temiz baslangic
if exist "SETUP_YouTube_Mezat_Yardimcisi_FIX3.zip" (
    del "SETUP_YouTube_Mezat_Yardimcisi_FIX3.zip"
)
...
if not exist "dist\run_as_admin.bat" (
...
if not exist "dist\KURULUM_KILAVUZU.txt" (
...
powershell -Command "Compress-Archive -Path 'dist\*' -DestinationPath 'SETUP_YouTube_Mezat_Yardimcisi_FIX3.zip' -Force"

if not exist "SETUP_YouTube_Mezat_Yardimcisi_FIX3.zip" (
    echo Hata: Zip dosyasi olusturulamadi!
    goto :end
)

echo Islem tamamlandi!
echo Dagitim paketi: SETUP_YouTube_Mezat_Yardimcisi_FIX3.zip
