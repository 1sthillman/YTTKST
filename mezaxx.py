...
def validate_license_code(channel, code, url=None):
    try:
        ...
        # Hata ayıklama için log kaydı ekle
        logging.info(f"Lisans doğrulama: Kanal={channel}, Kod={code}")
        
        # Lisans kodunu temizle (boşlukları kaldır)
        code = code.strip()
        ...
        if not supabase:
            # Supabase bağlantısı başarısız olursa yerel doğrulamaya güven
            channel_licenses = {k.lower(): v for k, v in data.get("channel_licenses", {}).items()}
            if channel.lower() in channel_licenses:
                return code in channel_licenses[channel.lower()], None
            return code in data.get("valid_codes", []), None
        
        # Kullanıcının IP adresini al
        ip_address = get_user_ip()
        
        # Kullanıcı zaten var mı kontrol et
        response = supabase.table("users").select("*").eq("youtube_channel", channel).execute()
        
        user_id = None
        if response.data:
            # Kullanıcı zaten var, aktif olup olmadığını kontrol et
            user = response.data[0]
            user_id = user["id"]
            
            # is_active değeri null veya false olabilir, her iki durumu da kontrol et
            is_active = user.get("is_active")
            if is_active is None or is_active is False:
                logging.warning(f"Kullanıcı {channel} devre dışı bırakılmış veya lisans süresi dolmuş")
                return False, None
                
            # Lisans kodunu güncelle ve son giriş zamanını güncelle
        ...
        # Lisans kullanımını kaydet
        if user_id:
            supabase.table("license_usage").insert({
                "user_id": user_id,
                "action": "login",
                "details": {
        ...
        # Yerel doğrulama sonucunu döndür ve user_id'yi de döndür
        channel_licenses = {k.lower(): v for k, v in data.get("channel_licenses", {}).items()}
        if channel.lower() in channel_licenses:
            valid_channel_codes = [c.strip() for c in channel_licenses[channel.lower()]]
            is_valid = code in valid_channel_codes
            logging.info(f"Kanal lisans doğrulama: {channel} - {is_valid}")
            return is_valid, user_id
        
        valid_codes = [c.strip() for c in data.get("valid_codes", [])]
        is_valid = code in valid_codes
        logging.info(f"Genel lisans doğrulama: {is_valid}")
        return is_valid, user_id
        
    except Exception as e:
        logging.exception("license")
        # Hata durumunda yerel doğrulamaya güven
        try:
            with open("license_codes.json", encoding="utf-8") as f:
                data = json.load(f)
            channel_licenses = {k.lower(): v for k, v in data.get("channel_licenses", {}).items()}
            if channel.lower() in channel_licenses:
                valid_channel_codes = [c.strip() for c in channel_licenses[channel.lower()]]
                is_valid = code in valid_channel_codes
                logging.info(f"Hata sonrası kanal lisans doğrulama: {channel} - {is_valid}")
                return is_valid, None
            
            valid_codes = [c.strip() for c in data.get("valid_codes", [])]
            is_valid = code in valid_codes
            logging.info(f"Hata sonrası genel lisans doğrulama: {is_valid}")
            return is_valid, None
        except:
            return False, None
...
class AuthScreen:
    ...
    def authenticate(self):
        ...
        # Supabase ile lisans kodunu doğrula
        is_valid, user_id = validate_license_code(channel, code, url)
        if not is_valid:
        ...
        auth = {
            "youtube_name": channel, 
            "youtube_url": url, 
            "authenticated": True,
            "supabase_user_id": user_id  # Supabase user_id'yi kaydet
        }
...
class ModernYouTubeMezatYardimcisi:
    def __init__(self, authorized_youtube_name):
        ...
        self.youtube_start_time = 0
        self.load_supabase_user_info()
    ...
    def start_youtube_stream(self):
        ...
        # Canlı yayın oturumunu Supabase'e kaydet
        self.log_stream_session(url)
    ...
    def stop_youtube_stream(self):
        ...
        # Canlı yayın oturumunu sonlandır
...
"""
    ...
    def direct_print(self, text):
        try:
            ...
            try:
                ...
                # Metni yazıcı tipine göre formatla
                text = printer_settings.format_text_for_printer(text)
            except Exception as e:
                logging.exception("Yazıcı ayarları yüklenemedi")
                # Yazıcı ayarları yüklenemezse orijinal metni kullan
    ...
    def load_supabase_user_info(self):
        """Supabase kullanıcı bilgilerini yükler ve gerçek zamanlı izleme başlatır."""
        try:
            # auth_data.json dosyasından Supabase kullanıcı ID'sini al
            if os.path.exists("auth_data.json"):
                with open("auth_data.json", "r", encoding="utf-8") as f:
                    auth_data = json.load(f)
                    self.supabase_user_id = auth_data.get("supabase_user_id")
                    
            # Supabase istemcisini başlat
            self.supabase_client = get_supabase_client()
            
            # Eğer Supabase kullanıcı ID'si ve istemci varsa, gerçek zamanlı izleme başlat
            if self.supabase_user_id and self.supabase_client:
                self.start_realtime_monitoring()
                
                # Periyodik kontrol zamanlayıcısını başlat
                self.start_periodic_check()
        except Exception as e:
            logging.exception("Supabase kullanıcı bilgileri yüklenirken hata oluştu")
            print(f"⚠️ Supabase kullanıcı bilgileri yüklenirken hata: {e}")
            
    def start_periodic_check(self):
        """Periyodik olarak kullanıcı durumunu ve lisans süresini kontrol eder."""
        try:
            # Her 5 dakikada bir kontrol et
            self.check_user_status()
            
            # Zamanlayıcıyı yeniden başlat (5 dakika = 300000 ms)
            self.root.after(300000, self.start_periodic_check)
            
            logging.info("Periyodik kontrol zamanlayıcısı başlatıldı")
            print("✅ Periyodik kontrol zamanlayıcısı başlatıldı (5 dakikada bir)")
        except Exception as e:
            logging.exception("Periyodik kontrol zamanlayıcısı başlatılırken hata oluştu")
            print(f"⚠️ Periyodik kontrol zamanlayıcısı başlatılırken hata: {e}")
            
    def log_stream_session(self, youtube_url):
        """Canlı yayın oturumunu Supabase'e kaydeder."""
        try:
            if not self.supabase_client or not self.supabase_user_id:
                logging.warning("Supabase bağlantısı yok, canlı yayın oturumu kaydedilemedi")
                return
            ...
            session_data = {
                ...
                "youtube_channel": self.authorized_youtube_name,
                "start_time": datetime.datetime.now().isoformat()
            }
            
            # Supabase'e kaydet
            response = self.supabase_client.table("stream_sessions").insert({
            ...
            if response.data:
                self.current_stream_session_id = response.data[0]["id"]
                logging.info(f"Canlı yayın oturumu başlatıldı ve kaydedildi: {self.current_stream_session_id}")
                print(f"✅ Canlı yayın oturumu başlatıldı ve kaydedildi: {self.current_stream_session_id}")
            else:
                logging.warning("Canlı yayın oturumu kaydedilemedi")
                print("⚠️ Canlı yayın oturumu kaydedilemedi")
                
        except Exception as e:
            logging.exception("Canlı yayın oturumu kaydedilirken hata oluştu")
            print(f"⚠️ Canlı yayın oturumu kaydedilirken hata: {e}")
            
    def end_stream_session(self):
        """Canlı yayın oturumunu sonlandırır ve Supabase'deki kaydı günceller."""
        try:
            if not hasattr(self, 'current_stream_session_id') or not self.current_stream_session_id:
                return
                
            if not self.supabase_client:
                logging.warning("Supabase bağlantısı yok, canlı yayın oturumu güncellenemedi")
                return
                
            # Oturum sonlandırma zamanını kaydet
            self.supabase_client.table("stream_sessions").update({
                "ended_at": datetime.datetime.now().isoformat(),
                "is_active": False,
                "session_data": {
                    "end_time": datetime.datetime.now().isoformat(),
                    "duration_seconds": time.time() - self.youtube_start_time if hasattr(self, 'youtube_start_time') else 0
                }
            }).eq("id", self.current_stream_session_id).execute()
            
            logging.info(f"Canlı yayın oturumu sonlandırıldı: {self.current_stream_session_id}")
            print(f"✅ Canlı yayın oturumu sonlandırıldı: {self.current_stream_session_id}")
            
            # Oturum ID'sini sıfırla
            self.current_stream_session_id = None
            
        except Exception as e:
            logging.exception("Canlı yayın oturumu sonlandırılırken hata oluştu")
            print(f"⚠️ Canlı yayın oturumu sonlandırılırken hata: {e}")
    
    def start_realtime_monitoring(self):
        """Supabase'de kullanıcı durumu değişikliklerini izlemeye başlar."""
        try:
            if not self.supabase_user_id:
                logging.warning("Supabase kullanıcı ID'si bulunamadı, gerçek zamanlı izleme başlatılamıyor")
                print("⚠️ Supabase kullanıcı ID'si bulunamadı, gerçek zamanlı izleme başlatılamıyor")
                return
                
            # Supabase Realtime istemcisini başlat
            channel = self.supabase_client.channel("user_status_changes")
            
            # PostgreSQL değişikliklerini dinle
            channel.on(
                "postgres_changes",
                event="UPDATE",
                schema="public",
                table="users",
                filter=f"id=eq.{self.supabase_user_id}",
                callback=self.handle_user_status_change
            )
            
            # Kanalı abone ol
            self.realtime_subscription = channel.subscribe()
            logging.info(f"Supabase gerçek zamanlı izleme başlatıldı (user_id: {self.supabase_user_id})")
            print(f"✅ Supabase gerçek zamanlı izleme başlatıldı (user_id: {self.supabase_user_id})")
            
            # Kullanıcı durumunu hemen kontrol et
            self.check_user_status()
        except Exception as e:
            logging.exception("Gerçek zamanlı izleme başlatılırken hata oluştu")
            print(f"⚠️ Gerçek zamanlı izleme başlatılırken hata: {e}")
    
    def check_user_status(self):
        """Kullanıcının aktif durumunu ve lisans süresini Supabase'den kontrol eder."""
        try:
            if not self.supabase_client or not self.supabase_user_id:
                return
                
            # Kullanıcı bilgilerini çek
            response = self.supabase_client.table("users").select("*").eq("id", self.supabase_user_id).execute()
            ...
            if response.data:
                ...
                # is_active değeri null veya false olabilir, her iki durumu da kontrol et
                if is_active is None or is_active is False:
                    # Kullanıcı devre dışı bırakılmışsa, ana ekrana geri dön
                    logging.warning("Kullanıcı devre dışı bırakılmış, oturum sonlandırılıyor...")
                    
                    # Ana thread'de çalıştır
                    self.root.after(0, lambda: self.logout_and_return_to_auth(expired=False))
                    
                    # Konsola da yazdır (debug için)
                    print("⚠️ Kullanıcı devre dışı bırakılmış, oturum sonlandırılıyor...")
                    
                # Lisans süresini kontrol et (eğer varsa)
                self.check_license_expiry(user)
        except Exception as e:
            logging.exception("Kullanıcı durumu kontrol edilirken hata oluştu")
            print(f"⚠️ Kullanıcı durumu kontrol edilirken hata: {e}")
            
    def check_license_expiry(self, user_data):
        """Kullanıcının lisans süresini kontrol eder."""
        try:
            # Lisans süresini kontrol et (eğer varsa)
            expiry_date = user_data.get("license_expiry")
            if not expiry_date:
                return
                
            # String tarihi datetime'a çevir
            try:
                expiry_datetime = datetime.datetime.fromisoformat(expiry_date.replace('Z', '+00:00'))
                now = datetime.datetime.now(datetime.timezone.utc)
                
                # Lisans süresi dolmuş mu kontrol et
                if now > expiry_datetime:
                    logging.warning("Lisans süresi dolmuş, oturum sonlandırılıyor...")
                    
                    # Ana thread'de çalıştır
                    self.root.after(0, lambda: self.logout_and_return_to_auth(expired=True))
                    
                    # Konsola da yazdır (debug için)
                    print("⚠️ Lisans süresi dolmuş, oturum sonlandırılıyor...")
                    return True
            except ValueError:
                logging.warning(f"Geçersiz tarih formatı: {expiry_date}")
                
            return False
        except Exception as e:
            logging.exception("Lisans süresi kontrol edilirken hata oluştu")
            print(f"⚠️ Lisans süresi kontrol edilirken hata: {e}")
            return False
    
    def handle_user_status_change(self, payload):
        """Kullanıcı durumu değiştiğinde çağrılır."""
        try:
            # Payload'dan yeni durum bilgisini al
            new_record = payload.get("new", {})
            is_active = new_record.get("is_active")
            
            # is_active değeri null veya false olabilir, her iki durumu da kontrol et
            if is_active is None or is_active is False:
                # Kullanıcı devre dışı bırakılmışsa, ana ekrana geri dön
                logging.warning("Kullanıcı devre dışı bırakıldı, oturum sonlandırılıyor...")
                
                # Ana thread'de çalıştır
                self.root.after(0, lambda: self.logout_and_return_to_auth(expired=False))
                
                # Konsola da yazdır (debug için)
                print("⚠️ Kullanıcı devre dışı bırakıldı, oturum sonlandırılıyor...")
                return
                
            # Lisans süresini kontrol et (eğer varsa)
            if "license_expiry" in new_record:
                self.check_license_expiry(new_record)
        except Exception as e:
            logging.exception("Kullanıcı durumu değişikliği işlenirken hata oluştu")
            print(f"⚠️ Kullanıcı durumu değişikliği işlenirken hata: {e}")
    
    def logout_and_return_to_auth(self, expired=False):
        """Kullanıcıyı çıkış yapar ve kimlik doğrulama ekranına geri döner."""
        try:
            # Kullanıcıya bilgi ver
            if expired:
                messagebox.showwarning(
                    "Lisans Süresi Doldu", 
                    "Lisans süreniz dolmuştur.\nLütfen yeniden lisans almak için iletişime geçin:\n\n"
                    "Telefon: 05439269670\nE-posta: sthillmanbusiness@gmail.com"
                )
            else:
                messagebox.showwarning(
                    "Oturum Sonlandırıldı", 
                    "Hesabınız yönetici tarafından devre dışı bırakıldı.\nLütfen yöneticinizle iletişime geçin:\n\n"
                    "Telefon: 05439269670\nE-posta: sthillmanbusiness@gmail.com"
                )
            ...
            logging.exception("Çıkış yapılırken hata oluştu")
            print(f"⚠️ Çıkış yapılırken hata: {e}")
            # Acil durum çıkışı
            sys.exit(0)
...
def start_auth_screen():
    """Kimlik doğrulama ekranını başlatır."""
    try:
        auth_screen = AuthScreen(on_success=start_main_app)
        auth_screen.run()
    except Exception as e:
        logging.exception("Kimlik doğrulama ekranı başlatılırken hata oluştu")
        messagebox.showerror("Hata", f"Kimlik doğrulama ekranı başlatılırken hata oluştu: {e}")
        sys.exit(0)
...
if __name__ == "__main__":
    try:
        # Supabase config dosyasını kontrol et, yoksa oluştur
        ...
        # Lisans kodları dosyasını kontrol et, yoksa oluştur
        ...
        if os.path.exists("auth_data.json"):
            try:
                ...
                if auth.get("authenticated"):
                    # Kullanıcı daha önce kimlik doğrulaması yapmış, doğrudan ana uygulamayı başlat
                    start_main_app(auth["youtube_name"])
                else:
                    # Kimlik doğrulama bilgileri geçersiz, kimlik doğrulama ekranını göster
